...
export default function ExamView() {
    ...
    // Intelligent Asset Loading (Windowed Preloading)
    // Only preload images for current question + next 3 to save bandwidth
    const windowedImages = exam?.questions
        ?.slice(currentQuestionIndex, currentQuestionIndex + 4)
        .map(q => q.media_url)
        .filter((url): url is string => !!url) || [];
    useImagePreloader(windowedImages);
    ...
    const fetchExam = async () => {
        try {
            const cacheKey = `durrah_exam_content_${id}`;
            ...
            try {
                ...
                if (cachedString) {
                    ...
                    if (cachedData?.exam && cachedData?.questions) {
                        setExam(cachedData.exam);
                        // Also set availability based on cached settings immediately to prevent flicker
                        checkAvailability(cachedData.exam.settings);
                        shouldFetchFull = false; // Tentatively assume cache is good
                    }
                }
            } catch (e) {
                console.warn('Failed to load exam cache', e);
            }

            // 2. Check version/timestamp from server (Lightweight check)
            // We only select 'updated_at' and 'settings' to keep it tiny.
            const { data: remoteExamHeader, error: headerError } = await supabase
                .from('exams')
                .select('updated_at, settings, is_active')
                .eq('id', id)
                .single();

            if (headerError) throw headerError;

            // If kids mode, skip portal check (from original logic)
            if (remoteExamHeader.settings?.child_mode_enabled) {
                sessionStorage.setItem('durrah_exam_portal_access', '1');
            }

            // 3. Compare timestamps to decide if we need a full heavy fetch
            // If we have no cache, OR if server has a newer updated_at, OR if forced refresh
            const remoteTime = new Date(remoteExamHeader.updated_at || 0).getTime();
            const cachedTime = cachedData?.timestamp || 0;

            // If local cache is missing or older than server, we MUST fetch
            if (!cachedData || remoteTime > cachedTime) {
                console.log('📦 Cache miss or stale. Fetching fresh exam data...');
                shouldFetchFull = true;
            } else {
                console.log('📦 Cache hit! Using local exam data.');
            ...
            if (shouldFetchFull) {
                ...
                // Process/Randomize Questions (logic reused)
                ...
                const fullExamObject = { ...examData, questions: processedQuestions, settings: normalizedSettings };
                setExam(fullExamObject);
                checkAvailability(normalizedSettings); // Helper function we'll create or inline

                // Update Cache
                try {
                    localStorage.setItem(cacheKey, JSON.stringify({
                        exam: fullExamObject,
                        questions: processedQuestions, // Redundant but keeps structure clear
                        timestamp: remoteTime > 0 ? remoteTime : Date.now(),
                        version: '1.0'
                    }));
                } catch (e) {
                    console.warn('Quota exceeded, could not cache exam', e);
                }

                // Initial timer set (only if not restored)
    ...
    // Helper to check availability (extracted to reuse)
    const checkAvailability = (settings: any) => {
        const now = new Date();
        let start: Date | null = null;
        let end: Date | null = null;
        if (settings.start_time) {
            const d = new Date(settings.start_time);
            if (!isNaN(d.getTime())) start = d;
        }
        if (settings.end_time) {
            const d = new Date(settings.end_time);
    ...
    useEffect(() => {
        if (showAutoSubmitWarning && autoSubmitCountdown > 0) {
            ...
            // TRAFFIC JITTER IMPLEMENTATION
            // 1. Show "Submitting..." UI immediately so user knows it's working
            setIsSubmitting(true);

            // 2. Add random delay (0-15s) to stagger server load
            const jitterMs = Math.floor(Math.random() * 15000);
            console.log(`⏳ Staggering submission by ${jitterMs}ms`);
