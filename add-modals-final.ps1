...
# Find the last occurrence of </div>\s*</div\s*>\s*\);\s*}
$pattern = "(<ViolationModal[\s\S]*?/>)\s*(<div className=`"mt-8 text-center pb-8`">)"
...
$content = $content -replace $pattern, $modals

Set-Content $file $content -NoNewline

Write-Host "? Modals added successfully!" -ForegroundColor Green
