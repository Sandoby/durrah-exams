// Complete fix script - Run with: node complete-fix.js
const fs = require('fs');
const path = require('path');

const filePath = path.join(__dirname, 'frontend', 'src', 'pages', 'ExamView.tsx');
const backupPath = filePath + '.backup-final';

console.log('?? Applying FINAL fixes to ExamView.tsx...\n');
...
// Fix 1: Remove duplicate import (line 5)
console.log('?? Fix 1: Removing duplicate import...');
content = content.replace(
    /import { AlertTriangle, CheckCircle, Loader2, Save, Flag, LayoutGrid, Sun, Moon, Calculator as CalcIcon, Star, Eye } from 'lucide-react';\s*import { AlertTriangle, CheckCircle, Loader2, Save, Flag, LayoutGrid, Sun, Moon, Calculator as CalcIcon, Star, Eye, AlertCircle, X } from 'lucide-react';/,
...
// Fix 2: Remove duplicate useEffect closing
console.log('?? Fix 2: Fixing useEffect duplication...');
content = content.replace(
    /} else if \(timeLeft === 0 && !submitted && !isSubmitting\) {\s*}\s*else if \(timeLeft === 0 && !submitted && !isSubmitting && !showAutoSubmitWarning\) {/,
    "} else if (timeLeft === 0 && !submitted && !isSubmitting && !showAutoSubmitWarning) {"
);

content = content.replace(
    /}\s*}, \[started, timeLeft, exam, submitted, isSubmitting\]\);\s*}, \[showAutoSubmitWarning, autoSubmitCountdown\]\);/,
...
// Fix 3: Remove duplicate button code
console.log('?? Fix 3: Cleaning up duplicate buttons...');
content = content.replace(
    /<button\s+type="submit"\s+disabled={isSubmitting}\s*<button type="button" onClick={handleSubmitWithCheck} disabled={isSubmitting}/g,
    '<button type="button" onClick={handleSubmitWithCheck} disabled={isSubmitting}'
);

// Fix 4: Add modals before final closing tags
console.log('?? Fix 4: Adding modal components...');
...
// Find the location to insert modals (before final closing tags)
const insertPoint = content.lastIndexOf('</div>\n        </div >');
if (insertPoint !== -1) {
    content = content.slice(0, insertPoint) + modalsCode + '\n            ' + content.slice(insertPoint);
}
...
console.log('  ? Cleaned up duplicate buttons');
console.log('  ? Added both modal components');
console.log('\n?? Now run: cd frontend && npm run build');
