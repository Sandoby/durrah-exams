#!/usr/bin/env node
// Auto-apply all ExamView.tsx changes
// Run: node apply-changes.js
...
console.log('?? Applying ExamView.tsx Changes...\n');
...
console.log('?? Change 1: Adding AlertCircle and X icons...');
...
console.log('?? Change 2: Adding state variables...');
const stateVars = `
...
console.log('?? Change 3: Updating timer logic...');
content = content.replace(
...
console.log('?? Change 4: Adding countdown effect...');
...
console.log('?? Change 5: Adding handleSubmitWithCheck function...');
const checkFunction = `
...
console.log('?? Change 6: Adding progress calculation...');
const progressCalc = `
...
console.log('?? Change 7: Updating submit buttons...');
content = content.replace(
    /<button\s+type="submit"\s+disabled={isSubmitting}/g,
    '<button type="button" onClick={handleSubmitWithCheck} disabled={isSubmitting}'
);

// Change 8: Add progress bar (find the header section)
console.log('?? Change 8: Adding progress bar...');
...
// Find the header closing tags
content = content.replace(
    /(<button\s+onClick=\{\(\) => setIsZenMode\(!isZenMode\)\}[\s\S]*?<\/button>\s*<\/div>\s*<\/div>\s*<\/div>)/,
    (match) => match.replace(
        /(<\/div>\s*<\/div>\s*<\/div>)$/,
        progressBar + '\n                        </div>\n                    </div>'
    )
...
// Change 9: Add modals at the end
console.log('?? Change 9: Adding modal components...');
const modals = `
...
console.log('  3. If errors, restore backup: copy frontend\\src\\pages\\ExamView.tsx.backup frontend\\src\\pages\\ExamView.tsx\n');
console.log('?? Done! Your exam now has all 3 features!');
